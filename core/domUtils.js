...
async function readDomComments(view) {
  const script = [
    ...
    '  const result = [];',
    '  const seen = new Set();',
    '  const SKIP = new Set(["回复","分享","作者赞过","收起","展开","举报","复制","删除","赞","踩","抢沙发","添加表情","查看更多回复","查看全部回复","更多回复","抢首评","作者","展开回复","展开\\d+条回复","作者\\u00b7","分享\\n","回复\\n","\\d+条回复"]);',
    ...
    '      if (/^[\\d\\.]+万?$/.test(t)) continue;',
    '      if (/^\\d+[秒分时天周月年]/.test(t)) continue;',
    '      if (/\\d+[月天周年前]+/.test(t) && t.length < 15) continue;',
    '      if (/^·/.test(t)) continue;',
    '      if (t.includes("分享") || t.includes("回复") || t.includes("展开") || t.includes("折叠")) continue;',
    ...
    '    // 提取IP属地',
    '    const ipMatch = item.innerText.match(/(\\d+)月前·([^\\s]+)/);',
    '    if (ipMatch) ip = ipMatch[2];',
    '    result.push({ text: best, nickname: nick, ip_label: ip, comment_id: "dom_" + Math.random().toString(36).substr(2, 9) });',
    '  }',
    '  return result;',
    '})()'
  ].join('\n');
